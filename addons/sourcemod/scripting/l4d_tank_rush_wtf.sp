...
FreezePoints( bool:show_message = false ) {
    if ( !bTankAlive ) {
        ...
        if ( show_message ) CPrintToChatAll("{red}[{default}坦克在你冲你马呢{red}] {red}坦克 {default}已生成. {olive}冻结 {default}距离分数!");
...
UnFreezePoints( bool:show_message = false ) {
    if ( bTankAlive ) {
        if ( show_message ) CPrintToChatAll("{red}[{default}坦克在你冲你马呢{red}] {red}坦克 {default}已死亡. {olive}恢复 {default}距离分数!");
